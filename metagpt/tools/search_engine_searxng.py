...
class SearXNGWrapper(BaseModel):
    model_config = ConfigDict(arbitrary_types_allowed=True)

    api_key: str
    bing_url: str = "http://localhost:8888/search"
    ...
    async def run(
        ...
        if DEBUG:
            logger.info("SearXNG running")
        result = await self.results(params)
        if DEBUG:
            logger.info(f'SearXNG run res:{result}')
        ...
        for item_dict in search_results:
            ...
            item_dict["snippet"] = item_dict["content"]
